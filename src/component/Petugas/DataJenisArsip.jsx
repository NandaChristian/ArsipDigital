...
export default function DataJenisArsip() {
    ...
    const handleEdit = (jenisItem) => {
        setIsEdit(true);
        setCurrentUuid(jenisItem.uuid);
        setJenis({
            name: jenisItem.name
        });
    };
    const handleDelete = async (jenisItem) => {
        if (window.confirm("Peringatan: Menghapus item ini akan menghapus semua sub-item di dalamnya!")) {
            try {
                const response = await fetch(import.meta.env.VITE_API_URL + '/api/jenis/' + jenisItem.uuid, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                                ...
                                <div className="card-body-master">
                                    <div className="col d-flex justify-content-center">
                                        <button
                                            ...
                                            data-bs-target="#tambahJenisArsip"
                                        >
                                            Tambah +
                                        </button>
                                    </div>
                                ...
                                <div className="modal-body">
                                    ...
                                    <form onSubmit={handleSubmit}>
                                        <div className="mb-3">
                                            ...
                                            <input type="text" value={jenis.name} onChange={(e) => setJenis({ name: e.target.value })} name="name" className="form-control radius-30" placeholder="Masukkan Tipe Arsip" />
                                        </div>
                                        <div className="d-flex align-items-center pb-0 pt-0 gap-3">
                                            <div className="w-50">
